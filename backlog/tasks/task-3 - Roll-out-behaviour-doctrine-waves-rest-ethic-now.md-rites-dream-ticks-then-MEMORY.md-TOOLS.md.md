...
- [x] #4 Wave two drafted and rolled out (same canary gate): capped ~40-line MEMORY.md with @MEMORY.md import, TOOLS.md template stub, existing agents seeded via the first RITE.md push
...
<!-- SECTION:NOTES:BEGIN -->
Closed 2026-07-14. AC4 verified complete: all six live agents carry MEMORY.md + TOOLS.md, each @-imported by CLAUDE.md (checked via GitHub Contents API). Wave two was canaried on rahel (the rite's relocation step needed a drifted CLAUDE.md to exercise; mina's only drift was procedural), the canary caught the rite-v1 defect (silent delete step, fixed in d4ca85f), then fanned out. The admin CLAUDE.md architecture note (last REMAINING item) is now committed. All ACs met.
<!-- SECTION:NOTES:END -->
